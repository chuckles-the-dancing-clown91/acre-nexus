...
pub async fn run(
    ...
    // Give the flagship LLC ready-to-use branding + default templates so lease /
    // letter generation works out of the box in the demo.
    seed_llc_onboarding(property_db, northwind, maple, "Maple Holdings LLC").await?;
...
async fn seed_llc(
    ...
    entity::llc::ActiveModel {
        ...
        website: Set(None),
        status: Set("active".into()),
        onboarded_at: Set(Some(now.into())),
        created_at: Set(now.into()),
        updated_at: Set(now.into()),
    }
    .insert(db)
...
/// Seed an LLC's branding + default lease/letter templates so document
/// generation is immediately usable in the demo.
async fn seed_llc_onboarding(
    db: &DatabaseConnection,
    tenant_id: Uuid,
    llc_id: Uuid,
    company_name: &str,
    ...
    let now = Utc::now();
    entity::llc_branding::ActiveModel {
        id: Set(Uuid::new_v4()),
        tenant_id: Set(tenant_id),
        llc_id: Set(llc_id),
        logo_document_id: Set(None),
        primary_color: Set(Some("#F5451F".into())),
        accent_color: Set(Some("#1C7C53".into())),
        signature_name: Set(Some("Jordan Avery".into())),
        signature_title: Set(Some("Managing Member".into())),
        signature_block: Set(Some(format!(
            "Sincerely,\n\n_____________________________\nJordan Avery, Managing Member\n{company_name}"
        ))),
        letterhead: Set(Some(format!("{company_name}\nProperty Management Office"))),
        footer: Set(Some(format!(
            "{company_name} — generated by Acre Nexus. This document is confidential."
        ))),
        created_at: Set(now.into()),
        updated_at: Set(now.into()),
    }
    ...
    for (kind, name, subject, body) in [
        (
            ...
            None,
            crate::templating::DEFAULT_LEASE_TEMPLATE,
        ),
        (
            "tenant_letter",
            "General Notice to Tenant",
            Some("A notice regarding your tenancy"),
            crate::templating::DEFAULT_LETTER_TEMPLATE,
        ),
    ] {
        entity::llc_template::ActiveModel {
            id: Set(Uuid::new_v4()),
            tenant_id: Set(tenant_id),
            llc_id: Set(llc_id),
            kind: Set(kind.into()),
            name: Set(name.into()),
            subject: Set(subject.map(String::from)),
            body: Set(body.into()),
            is_default: Set(true),
            created_at: Set(now.into()),
            updated_at: Set(now.into()),
