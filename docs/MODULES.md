...
| `integrations` | Integrations | on | `/integrations/secrets`, `/integrations/providers`, `/integrations/notifications`, `/integrations/inbound-emails`, `/notifications/*` (inbox + web push), `/documents`, `/webhooks/{provider}` (incl. `inbound_email` routing) + the `auto_email`/`auto_sms`/`auto_push`/`auto_chat`/`webhook_event`/`document_retention` jobs (see `docs/INTEGRATIONS.md`, `docs/NOTIFICATIONS.md`, `docs/EMAIL.md`) |
