...
`leasing`, `vendor_api`, `theming`, `flips`, `rehab`) so the two halves always
...
| `rentals` | Rentals & Leasing | on | `/properties/{id}/units`, `/leases`, `/leases/{id}/payments`, `/leases/{id}/inspections` + `/inspections/*`, `/leases/{id}/deposit` + `/deposit-dispositions/*`, `/my/inspections` + `/my/deposit` (see `docs/RENTALS.md`, `docs/PORTAL.md`) |
| `accounting` | Accounting & Payments | on | `/accounting/*` (ledger + reports), `/payments`, `/my/lease` + `/my/payments` + `/my/payment-methods` + `/my/autopay`, `/bank-accounts/*` + `/bank-transactions/*`, `/payouts`, `/payables` (vendor bills → approval → pay), `/finance/series` + the `billing_cycle`/`payment_process`/`bank_feed_sync`/`payout_execute`/`vendor_bill_pay`/`deposit_refund` jobs (see `docs/PAYMENTS.md`) |
| `calendar` | Calendar & Reminders | on | `/reminders` (lease renewals auto-synced, license/insurance expirations, tours, inspections) + the per-tenant `reminder_scan` job (see `docs/CALENDAR.md`) |
| `maintenance` | Maintenance & Work Orders | on | `/tickets`, `/properties/{id}/tickets`, `/tickets/{id}/comments`, `/tickets/{id}/quotes` + `/ticket-quotes/*`, `/maintenance-plans`, `/assets` (equipment registry), `/my/tickets` (resident requests + photos) + the per-tenant `helpdesk_scan` job (SLA breaches + preventive plans) (see `docs/RENTALS.md`, `docs/PORTAL.md`, `docs/HELPDESK.md`) |
| `messaging` | Resident Messaging | on | `/messages` (console), `/my/messages` (portal) — resident ↔ manager threads (see `docs/PORTAL.md`) |
| `title` | Title & Ownership | on | `/properties/{id}/ownership`, `/properties/{id}/liens` (see `docs/RENTALS.md`) |
| `leasing` | Leasing & Listings | on | `/public/*`, `/applications`, `/applications/{id}/screening` + `/adverse-action`, `/leads` (CRM, fed by inbound email) + the FCRA screening jobs (see `docs/SCREENING.md`, `docs/EMAIL.md`) |
