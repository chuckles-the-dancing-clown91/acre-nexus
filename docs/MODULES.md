...
| `maintenance` | Maintenance & Work Orders | on | `/tickets`, `/properties/{id}/tickets`, `/tickets/{id}/comments`, `/tickets/{id}/quotes` + `/ticket-quotes/*`, `/maintenance-plans`, `/my/tickets` (resident requests + photos) + the per-tenant `helpdesk_scan` job (SLA breaches + preventive plans) (see `docs/RENTALS.md`, `docs/PORTAL.md`, `docs/HELPDESK.md`) |
